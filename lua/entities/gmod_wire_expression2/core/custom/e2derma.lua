...
util.AddNetworkString("e2derma")
local function user_msg(tbl,k,pl,e2)
	net.Start("e2derma")
		net.WriteUInt(e2,16)
		net.WriteString(k)
		net.WriteTable(tbl)
...
util.AddNetworkString("e2derma_remove")
local function remove_derma(e2,pl)
	if !pl || !pl:IsValid() then return end
	net.Start("e2derma_remove")
		net.WriteUInt(e2,16)
	net.Send(pl)  
end

//***********basic sender************\\
local function sendDermaStuff(tbl,pl,e2,pod)
...
local function update_derma(e2,self, pod)
...
//**************************basic receiver************************\\
concommand.Add("_e2derma", function(player,commandName,args)
    if !args[1] or !args[2] then return end
    local e2     =  tonumber(args[1])
    local name   =  args[2]
    if !catch[e2] or !catch[e2].derma[name] then return end
	local func = rec[ catch[e2].derma[args[2]].type ]
	if !func then return end
	func(args)
    
    if catch[e2].derma[name].run_on_change then
        catch[e2].bexe = name
...
registerCallback("postexecute", function(self)
	if (self.dermacrap or {}).e2  == nil then return end
...
registerCallback("destruct", function(self)
	...
	local e2     = self.dermacrap.e2 
	if !catch[e2] then return end
	catch[e2].dontsend = true
	if !IsValid(catch[e2].pod) then
...
hook.Add( "PlayerEnteredVehicle", "derma_V_enter", function( pl, vehicle, role )
...
hook.Add("PlayerLeaveVehicle", "derma_V_Exit", function(pl, vehicle)
	for k,v in pairs(catch) do                                               
