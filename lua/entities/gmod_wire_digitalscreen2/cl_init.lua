...
net.Receive("wire_digitalscreen2", function(len)
	local ent = net.ReadEntity()
	if IsValid(ent) and ent.Memory1 and ent.Memory2 then
		local netReadUInt = net.ReadUInt
		for i=1,netReadUInt(20) do
			ent:WriteCell(netReadUInt(20),netReadUInt(24))
		end
	end
end)
net.Receive("wire_digitalscreen2_ordered", function(len)
	local ent = net.ReadEntity()
	if IsValid(ent) and ent.Memory1 and ent.Memory2 then
		local netReadUInt,entMemory2 = net.ReadUInt, ent.Memory2
		-- Lets streamline a bit from WriteCell, at a cost of readability
		ent:WriteCell(1048575,0)
		ent.IsClear = false
		local startaddr, numaddrs = netReadUInt(20), netReadUInt(20)
		for i=startaddr,startaddr+numaddrs do
			entMemory2[i] = netReadUInt(24)
		end
		ent:WriteCell(1048575,1)
		//timer.Create("wire_digitalscreen2_refresh_"..ent:EntIndex(),0.15,6,function() ent:WriteCell(1048575,1) end)
	end 
end) 
